...
DROP MATERIALIZED VIEW IF EXISTS bloodgasfirst3dayarterial CASCADE;
CREATE MATERIALIZED VIEW bloodgasfirst3dayarterial AS
with stg_spo2 as
(
...
(
select bg.*
  , ROW_NUMBER() OVER (partition by bg.icustay_id, bg.t_day, bg.charttime order by s1.charttime DESC) as lastRowSpO2
  , s1.spo2
from bloodgasfirst3day bg
...
(
select bg.*
  , ROW_NUMBER() OVER (partition by bg.icustay_id, bg.t_day, bg.charttime order by s2.charttime DESC) as lastRowFiO2
  , s2.fio2_chartevents
...
select subject_id, hadm_id, icustay_id, charttime, t_day
