...
DROP MATERIALIZED VIEW IF EXISTS bloodgasfirst3day CASCADE;
create materialized view bloodgasfirst3day as
with pvt as
( -- begin query that extracts the data
  select ie.subject_id, ie.hadm_id, ie.icustay_id
    , case 
          when le.charttime BETWEEN (ie.intime - interval '6' hour) AND (ie.intime + interval '1' day) Then '1'
          when le.charttime BETWEEN (ie.intime + interval '1' day) AND (ie.intime + interval '2' day) Then '2'
          when le.charttime BETWEEN (ie.intime + interval '2' day) AND (ie.intime + interval '3' day) Then '3'
          ELSE null
        end as t_day
  ...
  -- this also fuses together multiple ITEMIDs containing the same data
    ...
    left join labevents le
      ...
      and le.charttime between (ie.intime - interval '6' hour) and (ie.intime + interval '3' day)
      and le.ITEMID in
      -- blood gases
      (
...
select pvt.SUBJECT_ID, pvt.HADM_ID, pvt.ICUSTAY_ID, pvt.CHARTTIME, pvt.t_day
...
from pvt
WHERE t_day is not null
group by pvt.subject_id, pvt.hadm_id, pvt.icustay_id, pvt.CHARTTIME, pvt.t_day
order by pvt.subject_id, pvt.hadm_id, pvt.icustay_id, pvt.CHARTTIME, pvt.t_day;
