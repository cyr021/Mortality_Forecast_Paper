...
DROP MATERIALIZED VIEW IF EXISTS gcsfirst3day CASCADE;
create materialized view gcsfirst3day as
with base as
(
  ...
  select l.ICUSTAY_ID
  , case 
        when l.charttime BETWEEN (b.intime - interval '6' hour) AND (b.intime + interval '1' day) Then '1'
        when l.charttime BETWEEN (b.intime + interval '1' day) AND (b.intime + interval '2' day) Then '2'
        when l.charttime BETWEEN (b.intime + interval '2' day) AND (b.intime + interval '3' day) Then '3'
        ELSE null
      end as t_day
  ...
  -- Only get data for the first 72 hours
  and l.charttime between b.intime and b.intime + interval '3' day
  -- exclude rows marked as error
  and l.error IS DISTINCT FROM 1
  ) pvt
  group by pvt.ICUSTAY_ID, pvt.charttime, t_day
)
, gcs as (
  select b.*
...
, gcs_final as (
  ...
  , ROW_NUMBER ()
          OVER (PARTITION BY gcs.ICUSTAY_ID, t_day
                ORDER BY gcs.GCS
               ) as IsMinGCS
  from gcs
)
select ie.SUBJECT_ID, ie.HADM_ID, ie.ICUSTAY_ID, t_day
